...
public abstract class DynamicFunctor(DynamicType typeContext, MethodBuilder methodBuilder)
    : DynamicMethod(typeContext, methodBuilder.GetILGenerator())
{
    ...
    public void Return(ISymbol result)
    {
        if (!BuildingMethod.ReturnType.IsByRef)
            result.EmitLoadAsValue();
        else
            result.EmitLoadAsReference();
        Code.Emit(OpCodes.Ret);
    }
}

public class InstanceDynamicFunctor(DynamicType typeContext, MethodBuilder methodBuilder)
    : DynamicFunctor(typeContext, methodBuilder)
{
    [field: MaybeNull]
...
public class StaticDynamicFunctor(DynamicType typeContext, MethodBuilder methodBuilder)
    : DynamicFunctor(typeContext, methodBuilder)
{
